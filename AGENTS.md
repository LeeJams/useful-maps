...
`쓸모지도(Useful Maps)`는 복잡한 개발 개념과 실전 팁을 **왜 필요한지 → 어떻게 작동하는지 → 직접 비교하기 → 실전에 적용하기** 순서로 보여 주는 인터랙티브 시각 가이드 라이브러리다.
...
- 외부 기여자가 작은 Pull Request로도 안전하게 가이드를 넓힐 수 있게 한다.
...
- 한 PR은 하나의 새 가이드 또는 하나의 명확한 개선에 집중한다.
- 새 가이드는 구현 전에 핵심 질문, 비교할 선택지, 대표 인터랙션을 먼저 정의한다.
- 기술적 사실과 명령어를 바꾸는 경우 공식 문서나 재현 가능한 근거를 확인한다.
- 대규모 포맷 변경, 의존성 교체, 관련 없는 리팩터링을 콘텐츠 기여와 섞지 않는다.
- 시각 변경에는 리뷰어가 전후 차이를 확인할 수 있는 스크린샷이나 짧은 영상을 준비한다.
- 검증 방법은 다른 환경에서도 반복할 수 있게 명확히 남긴다.
- README는 기여자가 시작하는 문서로 유지한다. 내부 구현 제약을 모두 README에 복제하지 말고 세부 규칙은 이 파일에서 관리한다.
...
- PR 설명에 변경 목적, 확인 방법, 시각 변경 자료가 필요한가?
